...
def transform(ko_html: str, slug: str, blog_filename: str) -> str:
    ...
    # The optional wrapper div's margin style varies ("0 0 12px" on tool pages, "12px 0" seen
    # on some blog posts) -- both must be recognized together with the trailing optional close,
    # otherwise the wrapper's open tag is orphaned when its own </div> gets consumed by the
    # match's unconditional trailing (?:</div>)? group.
    AD_COUPANG_SWAP_RE = re.compile(
        r'(?:<div style="margin:(?:0 0 12px|12px 0)">)?'
